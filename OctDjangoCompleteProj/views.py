...
def add_products(request):
    if request.method == "POST":
        ...
        prod_qtty = request.POST.get('prod-qtty')
        prod_price = request.POST.get('prod-price')
        prod_desc = request.POST.get('prod-descr')
...
def delete(request, id):
    ...
    return redirect('view-pros-url')

def update_product(request, id):
    product = Product.objects.get(id=id)
    context = {'product': product}
    if request.method == "POST":
        updated_name = request.POST.get('prod-name')
        updated_qtty = request.POST.get('prod-qtty')
        updated_price = request.POST.get('prod-price')
        updated_descr = request.POST.get('prod-descr')

        product.name = updated_name
        product.qtty = updated_qtty
        product.price = updated_price
        product.descr = updated_descr
